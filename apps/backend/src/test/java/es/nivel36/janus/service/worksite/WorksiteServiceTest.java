...
class WorksiteServiceTest {
    ...
    @BeforeEach
    void setUp() {
        this.worksiteService = new WorksiteService(
    ...
    void assertEmployeeCanUseWorksiteShouldAllowAssignedScopeWhenEmployeeIsAssigned() {
        ...
        when(
            this.employeeService.isAssignedToWorksite(
                "aferrer@nivel36.es",
                "BCN-PROJ"
            )
        ).thenReturn(true);
        ...
        assertDoesNotThrow(() ->
            this.worksiteService.assertEmployeeCanUseWorksite(
                employee.getEmail(),
                worksite
            )
        );
        verify(this.employeeService).isAssignedToWorksite(
            "aferrer@nivel36.es",
            "BCN-PROJ"
        );
    }
