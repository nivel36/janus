...
/**
 * Represents an immutable half-open time interval {@code [start, end)} defined
 * by two {@link Instant} values.
 *
 * <p>
 * The interval includes the {@code start} instant and excludes the {@code end}
 * instant. Instances of this class are immutable and thread-safe.
 *
 * <p>
 * This class provides utility methods to determine temporal relationships
 * between intervals (such as overlap or adjacency), to merge or intersect
 * intervals, and to derive new intervals based on an existing one.
 */
class TimeInterval {
	 ...
	 *
	 * <p>
	 * Two intervals touch if the end of one interval is exactly equal to the start
	 * of the other, without overlapping.
	 *
	...
	/**
	 * Determines whether this interval either overlaps or touches the specified
	 * interval.
	 *
	 * @param other the interval to test
	 * @return {@code true} if the intervals overlap or touch, {@code false}
	 *         otherwise
	 * @throws NullPointerException if {@code other} is {@code null}
	 */
	 ...
	 *
	 * <p>
	 * The resulting interval spans from the earliest start instant to the latest
	 * end instant of both intervals.
	 *
	 * @param other the interval to merge with
	 * @return a new {@code TimeInterval} representing the merged interval
	 * @throws NullPointerException     if {@code other} is {@code null}
	 * @throws IllegalArgumentException if the intervals do not overlap or touch
	 */
	TimeInterval mergeWith(final TimeInterval other) {
		Objects.requireNonNull(other);
		if (!this.overlapsOrTouches(other)) {
	 ...
	 * @return a new {@code TimeInterval} representing the intersection, or
	 *         {@code null} if the intervals do not overlap
	...
	TimeInterval expandBy(final Duration margin) {
		...
		return new TimeInterval(this.start.minus(margin), this.end.plus(margin));
	}
	 ...
	 *
	 * @return the {@link Duration} between {@link #startsAt()} and
	 *         {@link #endsAt()}
	 */
	Duration duration() {
